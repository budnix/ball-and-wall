...
define(['./normalize'], function(normalize) {
    ...
    if (!engine) {} else if (engine[1] || engine[7]) {
        ...
        engine = 'gecko';
    } else if (testing) alert('Engine detection failed');
    ...
    cssAPI.addBuffer = function(resourceId) {
        ...
        if (indexOf(curBuffer, resourceId) != -1) return;
    ...
    cssAPI.setBuffer = function(css, isLess) {
        ...
        if (!baseUrl.match(absUrlRegEx)) baseUrl = '/' + baseUrl;
        ...
        for (var i = 0; i < curBuffer.length; i++) {
            ...
            if ((isLess && curBuffer[i].substr(curBuffer[i].length - 5, 5) == '.less') || (!isLess && curBuffer[i].substr(curBuffer[i].length - 4, 4) == '.css')) {
                (function(resourceId) {
                    ...
                    setTimeout(function() {
                        if (typeof onBufferLoad[resourceId] == 'function') onBufferLoad[resourceId]();
    ...
    cssAPI.linkLoad = function(url, callback) {
        ...
        // hacks
            ...
            if (engine == 'webkit') {
                ...
                head.appendChild(style);
            } else if (engine == 'trident') ieLoad(url, _callback);
        }
    }
    ...
    var get = function(url, callback, errback) {
        ...
        else if (typeof ActiveXObject !== 'undefined') for (i = 0; i < 3; i += 1) {
            progId = progIds[i];
            try {
    ...
    cssAPI.inject = function(css) {
        ...
        if (curStyle.styleSheet) curStyle.styleSheet.cssText += css;
        else curStyle.appendChild(document.createTextNode(css));
    }
    ...
    var loadCSS = function(fileUrl, callback, errback) {
        ...
        if (!fileUrl.match(absUrlRegEx)) fileUrl = '/' + normalize.convertURIBase(fileUrl, pathname, '/');
        ...
        get(fileUrl, function(css) {
            ...
            if (importUrls.length == 0) callback(css);
        }, errback);
    }
    ...
    cssAPI.load = function(cssId, req, load, config, parse) {
        ...
        if (cssAPI.attachBuffer(resourceId, load)) return;
        ...
        if (!parse) {
            ...
            loadCSS(fileUrl, function(css) {
                ...
                if (parse) css = parse(css, function(css) {
                    cssAPI.inject(css);
                    setTimeout(load, 7);
                });
            });
    ...
    if (testing) cssAPI.inspect = function() {
        if (stylesheet.styleSheet) return stylesheet.styleSheet.cssText;
        else if (stylesheet.innerHTML) return stylesheet.innerHTML;
    }
